...
namespace ManagerPortal
{
    public class Basepage
    {
        ...
        public async Task Write(IPage page, string loc, string text, string stepdetail)
        {
            try
            {
                ...
                if (locator != null)
                {
                    ...
                    Console.WriteLine($"{stepdetail} successfully");
        ...
        public async Task Type(IPage page, string loc, string text, string stepdetail)
        {
            try
            {
                var locator = await page.WaitForSelectorAsync(loc);
                if (locator != null)
                {
        ...
        public async Task SelectDropDown(IPage page, string loc, string detail)
        {
            try
            {
                ...
                var dropdown = await page.WaitForSelectorAsync(loc);
        ...
        public async Task SelectFilter(IPage page, string loc, string searchloc, string text, string detail)
        {
            
         // Wait for the dropdown element to be fully loaded
                await page.WaitForSelectorAsync(loc);
         // Click on the dropdown element
                await SimpleClick(page, loc);
         // Wait for the search element to be fully loaded
                await page.WaitForSelectorAsync(searchloc);
         // Perform keyboard actions to navigate the dropdown options
                await Write(page, searchloc, text, detail);
                await Keyboardaction(page, "down");
                await Keyboardaction(page, "enter");
        ...
        public async Task SelectProject(IPage page, string loc, string searchloc, string text , string resultloc, string detail)
        {
            try
            {
                // Query the dropdown element using the provided locator
                ...
                await SimpleClick(page, resultloc);
                await Wait(5000);


                // Log the action performed for the given detail
                Console.WriteLine($"{detail} successfully");
            }
            catch (Exception ex)
            {
                await extent.TakeScreenshot(page, Status.Fail, detail);
